...
install_base() {
    if [[ x"${release}" == x"centos" ]]; then
        yum install wget lolcat curl tar -y
    else
        apt install wget lolcat curl tar -y
    fi
}
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://api.github.com/repos/4xmen/x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-${arch}.tar.gz https://github.com/4xmen/x-ui/releases/download/${last_version}/x-ui-linux-${arch}.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/4xmen/x-ui/releases/download/${last_version}/x-ui-linux-${arch}.tar.gz"
    ...
    wget --no-check-certificate -O /usr/bin/x-ui https://raw.githubusercontent.com/4xmen/x-ui/main/x-ui.sh
...
⣿⠀⢀⣠⣾⣿⣿⠟⠁⠀⠀⠀⠀⠀⠀⠻⣿⡿⣿⣆⠀⠀⣿⠀⠀⠀⠀⠀⠀⠀⠀x-ui v2-ui           - Migrate     v2-ui Account data to x-ui⠀⠀⣿⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
